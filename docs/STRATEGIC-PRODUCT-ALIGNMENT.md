...
| **5.6** Insurer/product/coverage catalog | **Done (MVP)** | **API:** `Insurer` + `Product` com `product_line`, coberturas (texto + JSONB), perfil indicado, argumentos, materiais (JSONB), `insurer_id`, `active`; `GET /v1/products` com `q`, `category`, `insurer_id`; `GET /v1/insurers/{id}/products`. **Web:** administração → Seguradoras (lista, criar, **detalhe/editar**, produtos da seguradora) e **Produtos (catálogo)** (lista com filtro por linha de seguro, criar/editar, desativar). **Still:** anexos binários, hierarquia formal de coberturas além de texto/JSON, governança editorial avançada — see [`PHASE-PRE6-MODULES-56-59.md`](./PHASE-PRE6-MODULES-56-59.md) |
